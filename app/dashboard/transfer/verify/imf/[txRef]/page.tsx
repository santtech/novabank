...
import { Loader2, Shield, ChevronLeft, CreditCard, Lock, Globe } from "lucide-react"
...
export default function IMFVerificationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!imfCode.trim()) {
      setError("Please enter the International Clearance Code")
      return
    }
    ...
    <div className="min-h-screen bg-[#F4F6FA] p-4 md:p-6 pt-16 lg:pt-6">
      <div className="max-w-2xl mx-auto space-y-5">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl md:text-5xl font-black text-slate-900 tracking-tighter italic">Global Verification</h1>
            <p className="text-sm md:text-base text-slate-400 font-bold uppercase tracking-widest opacity-60">Step 2 of 6: International Step</p>
          </div>
        </div>
          ...
          <div className="bg-white rounded-xl shadow-lg border border-slate-100 overflow-hidden">
            <div className="p-6 md:p-8 flex flex-col items-center text-center space-y-4">
              ...
              <div className="space-y-1">
                <h2 className="text-2xl md:text-3xl font-black text-slate-900 uppercase tracking-tight italic">Cross-Border <span className="text-orange-600">Approval</span></h2>
                <p className="text-sm md:text-base text-slate-500 font-bold uppercase tracking-widest opacity-60 max-w-sm">
                  Please provide your <span className="font-bold text-slate-700">Clearance Code (IMF)</span> to clear this transaction for international settlement.
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="w-full space-y-6 pt-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="imfCode" className="text-[10px] md:text-xs font-black text-slate-500 uppercase tracking-widest">Clearance Code</Label>
                  <Input
                    id="imfCode"
                    type="text"
                    value={imfCode}
                    onChange={(e) => setImfCode(e.target.value.toUpperCase())}
            ...
            <div className="bg-slate-50 p-4 border-t border-slate-100 flex items-center justify-center gap-2">
              ...
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Secured institutional clearance</span>
            </div>
          </div>
        </motion.div>
